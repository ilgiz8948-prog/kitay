...
export function subscribeToOrdersFromCloud(onData: (orders: any[]) => void, onError?: (err: any) => void) {
  const colRef = collection(db, 'marketplace_orders');
  return onSnapshot(colRef, (snapshot) => {
    const orders: any[] = [];
    snapshot.forEach(docSnap => {
    ...
    console.warn('Unable to subscribe to marketplace orders from cloud:', error?.message || error);
    if (onError) onError(error);
  });
}

export async function saveOrderToCloud(order: any) {
  try {
    const docRef = doc(db, 'marketplace_orders', order.id);
...
export async function loadOrdersFromCloud() {
  try {
    ...
    console.warn('Unable to load orders from cloud:', error?.message || error);
    return null;
  }
}
