...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubOrders = subscribeToOrdersFromCloud(
      (cloudOrders) => {
        if (cloudOrders) {
          setOrders(cloudOrders as MarketplaceOrder[]);
          saveLocal('sinocalc_orders_local', cloudOrders);
        }
      }
    );
  ...
  const loadAllData = async () => {
    ...
    const localOrders = getLocal<MarketplaceOrder[]>('sinocalc_orders_local', []);
    ...
    try {
      const cloudOrders = await withTimeout(loadOrdersFromCloud(), 8000);
      if (cloudOrders !== null) {
        setOrders(cloudOrders as MarketplaceOrder[]);
        saveLocal('sinocalc_orders_local', cloudOrders);
      }
    } catch (err) {
      console.warn('Orders cloud load timed out or failed:', err);
    }
  ...
  const triggerPushNotification = (title: string, body: string, orderId?: string) => {
  ...
  const handlePlaceOrder = async (newOrder: MarketplaceOrder) => {
    setOrders((prev) => [newOrder, ...prev]);
    const updated = [newOrder, ...orders];
    saveLocal('sinocalc_orders_local', updated);
    ...
    triggerPushNotification(
      `🎉 Заказ ${newOrder.orderNumber} зарегистрирован!`,
      `Сумма: ${newOrder.totalAmount.toLocaleString()} ${newOrder.currency === 'KGS' ? 'сом' : '$'}. Трек: ${newOrder.trackingNumber}`,
      newOrder.id
    );
  };
  ...
  const handleUpdateOrderStatus = async (
    ...
    const statusLabels: Record<OrderStatus, string> = {
      ...
      shipped: 'В пути / Транзит',
      at_pickup_point: 'Прибыл в пункт выдачи (ПВЗ)',
    ...
    if (updatedOrder) {
      const currentOrders = orders.map(o => o.id === orderId ? updatedOrder! : o);
      saveLocal('sinocalc_orders_local', currentOrders);
      try {
        await saveOrderToCloud(updatedOrder);
      } catch (e) {
        console.warn('Failed to save order status update to cloud:', e);
      }

      triggerPushNotification(
        `📦 Заказ ${(updatedOrder as MarketplaceOrder).orderNumber}: ${statusLabels[newStatus]}`,
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 text-slate-200 font-sans">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-400">
            ...
            <span>Подключение к облаку БИЗНЕС СКЛАД...</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex flex-col bg-slate-950 font-sans text-slate-200 antialiased">
        ...
        <div className="flex items-center gap-3">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800">
            ...
            <button
              ...
              className="relative px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold rounded-lg text-xs tracking-wide flex items-center gap-1.5 transition-all border border-slate-700 shadow"
            >
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-slate-900 border-b border-slate-800 p-4 space-y-2 flex flex-col z-30">
          ...
          <button
            onClick={() => { setShowMarketplaceView(true); setIsMobileMenuOpen(false); }}
            className="w-full py-3 px-4 rounded-xl text-left text-sm font-bold flex items-center justify-between bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg"
          >
            <span className="flex items-center gap-2">
          ...
          <button
            onClick={() => { setShowSellerOrdersModal(true); setIsMobileMenuOpen(false); }}
            className="w-full py-3 px-4 rounded-xl text-left text-sm font-bold flex items-center justify-between bg-slate-800 text-slate-200 border border-slate-700 shadow"
          >
            <span className="flex items-center gap-2">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-xl w-full max-h-[85vh] overflow-y-auto space-y-4">
            ...
            <div className="text-xs text-slate-300 space-y-3.5 leading-relaxed">
              <p>
                <strong>БИЗНЕС СКЛАД</strong> использует формулу <span className="text-blue-400">Landed Cost</span> (конечная цена товара на складе) для выявления чистой прибыли.
              </p>
      ...
      <ReceiptModal
        ...
        currencySymbol={calculations?.currencySymbol || (activeBatch?.targetCurrency === 'USD' ? '$' : 'сом')}
        storeName={activeBatch?.name ? `БИЗНЕС СКЛАД (${activeBatch.name})` : 'БИЗНЕС СКЛАД'}
      />
      ...
      {/* 10. SELLER ORDERS MANAGEMENT MODAL */}
      <SellerOrdersManager
        isOpen={showSellerOrdersModal}
      ...
      {activePushToast && (
        <div className="fixed top-5 right-5 z-50 bg-indigo-950 border border-indigo-500/50 text-white p-4 rounded-2xl shadow-2xl max-w-sm flex items-start gap-3 animate-in slide-in-from-top-5 duration-300">
          <div className="p-2 rounded-xl bg-indigo-600 text-white shrink-0">
            <Bell className="w-5 h-5" />
          </div>
          <div className="flex-1 text-xs">
            <h4 className="font-bold text-white">{activePushToast.title}</h4>
            <p className="text-slate-300 mt-1">{activePushToast.body}</p>
          </div>
          <button
